...
- `tests/test_install_filesystem.bats` now owns the filesystem conflict boundary and stays maintained.
- `tests/test_install_metadata_copy.bats` now owns required/optional metadata-copy boundaries, readability failures, and unusual-but-valid target-path coverage.
- `tests/test_install_environment.bats` remains transitional only for late-phase executable-verification, template-render, and final `install` handoff cases; early filesystem/project-sync ownership has moved out of the mixed file.
- `tests/test_install_project_sync.bats` now sources the project-sync success/failure and ordering cases, while one executable-verification-boundary scenario intentionally remains there until T03 completes the late-phase split.
...
| maintained | `tests/test_install_filesystem.bats` | `existing target directory conflict fails before mutation and suppresses handoff` | `filesystem` conflict blocks mutation and suppresses downstream handoff/completion. | `tests/test_install_filesystem.bats` | Moved out of the transitional environment bucket during T02. |
| maintained | `tests/test_install_project_sync.bats` | `project-sync non-zero exits with phase diagnostics and no completion output` | `project-sync` failure halts before `template-render` and suppresses handoff/completion. | `tests/test_install_project_sync.bats` | Moved out of the transitional environment bucket during T02. |
...
| maintained | `tests/test_install_metadata_copy.bats` | `present source uv.lock is copied into target metadata` | `metadata-copy` copies the optional lockfile into the target when the source lock exists. | `tests/test_install_metadata_copy.bats` | Moved out of `tests/test_install_project_sync.bats` during T02. |
...
| maintained | `tests/test_install_metadata_copy.bats` | `missing source pyproject.toml fails in metadata-copy before project-sync` | `metadata-copy` fails closed when the required manifest is missing. | `tests/test_install_metadata_copy.bats` | Moved out of `tests/test_install_project_sync.bats` during T02. |
| maintained | `tests/test_install_metadata_copy.bats` | `unreadable source pyproject.toml fails in metadata-copy before provisioning` | `metadata-copy` fails closed when the required manifest exists but is unreadable. | `tests/test_install_metadata_copy.bats` | Moved out of `tests/test_install_project_sync.bats` during T02. |
| maintained | `tests/test_install_metadata_copy.bats` | `unreadable source uv.lock fails in metadata-copy before project-sync` | `metadata-copy` fails closed when the optional lockfile exists but is unreadable. | `tests/test_install_metadata_copy.bats` | Moved out of `tests/test_install_project_sync.bats` during T02. |
| maintained | `tests/test_install_metadata_copy.bats` | `unusual but valid target paths with spaces still receive copied metadata` | `metadata-copy` handles valid target paths with spaces without weakening copied-file guarantees. | `tests/test_install_metadata_copy.bats` | Moved out of `tests/test_install_project_sync.bats` during T02. |
