...
@test "metadata-copy runs before sync and preserves canonical ddgs handoff path" {
  ...
  assert_contains "$skill_content" "state the queries used" "rendered skill should preserve query-reporting guidance"
  assert_contains "$skill_content" "fallback tooling was required" "rendered skill should preserve fallback disclosure guidance"
  ...
  assert_output_contains "[phase:install] Final MCP handoff snippet (copy under mcpServers in your MCP config):" "install should announce the final handoff block"
  ...
  assert_line_order "$output" "[phase:template-render] Rendered skill document: $target_skill_doc" "[phase:install] Final MCP handoff snippet (copy under mcpServers in your MCP config):" "handoff snippet should follow successful target-local render"
}
