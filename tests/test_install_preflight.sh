...
run_installer() {
  ...
  env -i \
    ...
    INSTALLER_UV_SYNC_CMD='mkdir -p "$INSTALLER_VENV_PATH/bin" && ln -sf "$(command -v python3)" "$INSTALLER_VENV_PATH/bin/python" && : > "$INSTALLER_DDGS_PATH" && chmod +x "$INSTALLER_DDGS_PATH" && : > "$INSTALLER_PROJECT_DIR/uv.lock"' \
    bash "$INSTALLER" "$@"
}
